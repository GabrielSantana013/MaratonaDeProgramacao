...
“Top 10” poderia indicar qualquer posição entre primeiro e décimo colocado, 
porém se ele estivesse em primeiro provavelmente teria dito “Top 1”, 
se estivesse em segundo ou terceiro provavelmente teria dito “Top 3”, 
e se estivesse em quarto ou quinto provavelmente teria dito “Top 5”. 
...
Saída
Para cada caso de teste imprima uma linha contendo a frase “Top N”, e substitua a letra N pelo número da menor 
categoria que a colocação K pertence.
*/

#include <iostream>

int main(){
    int K, tops[] = {1,3,5,10,25,50,100};
    std::cin>>K;
    int sizeVetor = sizeof(tops) / sizeof(tops[0]);
    for(int i = 0; i < sizeVetor; i++)
    {
        if(K<=tops[i]){std::cout<<"Top "<<tops[i]<<std::endl; break;}
        else if(K>=100){std::cout<<"Top 100"<<std::endl; break;}
    }
    return 0;
}